<template>
	<div class="container px-4" style="width: 100%">
		<div class="mt-5" style="width: 100%">
            ...
            <p class="">공연 기간: {{ formatter(concertInfo.startedAt) }} ~ {{ formatter(concertInfo.endedAt) }}</p>
...
<!--							</span>-->
		...
		<div class="reservation-form d-flex">
          ...
          <option selected value="-1" >날짜선택</option>
					<option :value="concertDate.concertDateId" v-for="(concertDate) in concertDates">{{formatter(concertDate.concertDate)}}</option>
				</select>
			</div>
...
import { getConcertById, getConcertRatingById, getConcertDates } from '@/api/concert.js';
...
(async () => {
  ...
  const concertRatingResponse = await getConcertRatingById(response.data.data.regionId);
  ...
  ticketPriceInfo.R = ticketPriceResponse.data.data.filter(x => x.ticketRating === 'R')[0].price;
  ticketPriceInfo.S = ticketPriceResponse.data.data.filter(x => x.ticketRating === 'S')[0].price;
  ticketPriceInfo.A = ticketPriceResponse.data.data.filter(x => x.ticketRating === 'A')[0].price;
